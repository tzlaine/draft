...
and finally erases the range \range{ranges::unique(*this,
key_equiv(compare))}{end()}.
...
\pnum
\effects Equivalent to the preceding constructors except that \tcode{c.keys} and \tcode{c.values} are constructed with
...
For any member that takes a \tcode{sorted_equivalent_t}, the equivalent member
of \tcode{flat_map} is the corresponding member that takes a
\tcode{sorted_unique_t}.
