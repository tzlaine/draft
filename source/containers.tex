...
that it:
\begin{itemize}
\item does not meet the requirements related to node handles\iref{container.node},
\item does not meet the requirements related to iterator invalidation\iref{container.adaptors.general}, and
\item the time complexity of the \tcode{insert}, \tcode{emplace}, \tcode{emplace_hint},
and \tcode{erase} members that take a single element to respectively insert,
emplace or erase from the map is linear, including the ones that take an
...
namespace std {
  ...
  public:
    ...
    friend void swap(const flat_map& x, const flat_map& y) noexcept(noexcept(x.swap(y))
      { return x.swap(y); }
...
\tcode{value_comp()}; and finally erases the range \range{ranges::unique(*this)}{end()};
...
finally erases the range \range{ranges::unique(*this)}{end()}.

\pnum
\complexity
Linear in $N$ if the container arguments are already sorted with respect to
\tcode{value_comp()} and otherwise $N \log N$, where $N$ is
\tcode{distance(first, last)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\requires \tcode{Alloc} shall satisfy the \oldconcept{Allocator} requirements
(\tref{utilities.allocator.requirements}).  Constructors that take an \tcode{Alloc}
argument shall participate in overload resolution only if
\tcode{uses_allocator_v<key_container_type, Alloc> \&\& uses_allocator_v<mapped_container_type, Alloc>} is \tcode{true}.
...
and \tcode{args..} are unchanged.  Otherwise, equivalent to:
\begin{codeblock}
...
\begin{itemdescr}
\pnum \expects
The range \range{first}{last} is sorted with respect to \tcode{value_comp()}.
...
\begin{codeblock}
try {
  containers result{std::move(c.keys), std::move(c.values)};
  return result;
} catch(...) {
  clear();
  throw;
...
namespace std {
  ...
  public:
    ...
    friend void swap(const flat_multimap& x, const flat_multimap& y) noexcept(noexcept(x.swap(y))
      { return x.swap(y); }
...
\end{codeblock}
and finally sorts the range \range{begin()}{end()} with respect to \tcode{value_comp()}.
