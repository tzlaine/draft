...
\pnum
For container adaptors that provide them, the \tcode{insert}, \tcode{emplace},
and \tcode{erase} members affect the validity of iterators and
...
\begin{example}
A call to \tcode{flat_map<Key, T>::insert} could invalidate all iterators to
the \tcode{flat_map}.
\end{example}
...
the \libconcept{RandomAccessIterator}\iref{iterator.concept.random.access} concept.

\pnum
A \tcode{flat_map} satisfies all of the requirements of a container and of a
reversible container\iref{container.requirements}.  \tcode{flat_map} satisfies
the requirements of an associative container\iref{associative.reqmts}, except
that:
\begin{itemize}
\item it does not meet the requirements related to node handles\iref{container.node},
\item it does not meet the requirements related to iterator invalidation\iref{container.adaptors.general}, and
...
position iterator.
\item \tcode{flat_map::reverse_iterator} and \tcode{flat_map::const_reverse_iterator}
need not be implemented using \tcode{reverse_iterator}
(though \tcode{flat_map::reverse_iterator}
and \tcode{flat_map::const_reverse_iterator} otherwise satisfy the
requirements of \ref{container.requirements}).
\end{itemize}
A \tcode{flat_multimap} does not meet the additional requirements of an
allocator-aware container, as described in \tref{containers.allocatoraware}.
...
\tcode{deque}\iref{deque} can be used.  \begin{note}\tcode{vector<bool>} is
...
namespace std {
  ...
  public:
    ...
    using reverse_iterator          = @\impdefx{type of \tcode{flat_map::reverse_iterator}}@; // see above
    using const_reverse_iterator    = @\impdefx{type of \tcode{flat_map::const_reverse_iterator}}@; // see above
...
\tcode{value_comp()}; and finally erases the range \range{ranges::unique(*this,
key_equiv(compare))}{end()};
...
\begin{itemdescr}
\pnum
\constraints \tcode{uses_allocator_v<key_container_type, Alloc> \&\&
uses_allocator_v<mapped_container_type, Alloc>} is \tcode{true}.

\pnum
\effects Equivalent to the preceding constructors except that \tcode{c.keys}
and \tcode{c.values} are constructed with uses-allocator
construction\iref{allocator.uses.construction}.
\end{itemdescr}
...
the range \range{ranges::unique(*this, key_equiv(compare))}{end()}.
...
range \range{ranges::unique(*this, key_equiv(compare))}{end()}.
...
\effects
If the map already contains an element whose key is equivalent to \tcode{k},
\tcode{*this}, \tcode{k}, and \tcode{args...} are unchanged.  Otherwise equivalent to:
\begin{codeblock}
auto key_it = lower_bound(c.keys.begin(), c.keys.end(), k);
auto value_it = c.values.begin() + distance(c.keys.begin(), key_it);
...
Otherwise equivalent to \tcode{emplace(k, std::forward<M>(obj))} or
\tcode{emplace_hint(hint, k, std::forward<M>(obj))} respectively.
...
Otherwise equivalent to \tcode{emplace(std::move(k), std::forward<M>(obj))} or
\tcode{emplace_hint(hint, std::move(k), std::forward<M>(obj))} respectively.
...
\pnum
A \tcode{flat_multimap} satisfies all of the requirements of a container and of a
reversible container\iref{container.requirements}.  \tcode{flat_map} satisfies
the requirements of an associative container\iref{associative.reqmts}, except
that:
\begin{itemize}
...
element from the map is linear, including the ones that take an insertion
position iterator.
\item \tcode{flat_multimap::reverse_iterator} and
\tcode{flat_multimap::const_reverse_iterator} need not be implemented using
\tcode{reverse_iterator} (though \tcode{flat_multimap::reverse_iterator} and
\tcode{flat_multimap::const_reverse_iterator} otherwise satisfy the requirements of
...
not remove elements with equal keys.  \begin{example}\tcode{flat_multimap}
constructors do not erase non-unique elements after sorting them.\end{example}
...
and \tcode{deque}\iref{deque} can be used.  \begin{note}\tcode{vector<bool>}
...
namespace std {
  ...
  public:
    ...
    using reverse_iterator          = @\impdefx{type of \tcode{flat_multimap::reverse_iterator}}@; // see above
    using const_reverse_iterator    = @\impdefx{type of \tcode{flat_multimap::const_reverse_iterator}}@; // see above
    using key_container_type        = KeyContainer;
    using mapped_container_type     = MappedContainer;
    ...
    template <class Container, class Alloc>
      flat_multimap(sorted_equivalent_t, const Container& cont, const Alloc& a);
...
reversible container\iref{container.requirements}.  \tcode{flat_set} satisfies
the requirements of an associative container\iref{associative.reqmts}, except
that:
\begin{itemize}
\item it does not meet the requirements related to node handles\iref{container.node},
...
is constructed with uses-allocator construction\iref{allocator.uses.construction}.
\end{itemdescr}
